...
describe('ConfigProperty', () => {
    ...
    describe('Basic functionality', () => {
        it('should create with default value', () => {
            const config = new ConfigProperty('test', 'defaultValue', undefined, undefined, mockStorageInterface);
        ...
        it('should save and load string values', () => {
            ...
            const config2 = new ConfigProperty('test', 'default', undefined, undefined, mockStorageInterface);
            expect(config2.value).toBe('newValue');
        });

        it('should save and load object values', () => {
            const defaultObj = { name: 'default', count: 0 };
            const config = new ConfigProperty('test', defaultObj, undefined, undefined, mockStorageInterface);

            const newObj = { name: 'updated', count: 42 };
            config.value = newObj;

            // Create a new instance to verify persistence
            const config2 = new ConfigProperty('test', defaultObj, undefined, undefined, mockStorageInterface);
        ...
        it('should validate values when validator is provided', () => {
            ...
            const config = new ConfigProperty<number>('test', 50, undefined, validator, mockStorageInterface);
        ...
        it('should call saveCallback when value is set', () => {
            ...
            const config = new ConfigProperty('test', 'default', saveCallback, undefined, mockStorageInterface);

            config.value = 'newValue';
            expect(saveCallback).toHaveBeenCalled();
        });

        it('should emit changes through Observable', async () => {
            const config = new ConfigProperty('test', 'initial', undefined, undefined, mockStorageInterface);
    ...
    describe('Time-based configuration', () => {
        ...
        it('should handle concurrent updates correctly', () => {
            ...
            const config1 = new ConfigProperty('shared', 'initial', undefined, undefined, mockStorageInterface);
            const config2 = new ConfigProperty('shared', 'initial', undefined, undefined, mockStorageInterface);
            ...
            const config2Reloaded = new ConfigProperty('shared', 'initial', undefined, undefined, mockStorageInterface);
            ...
            const config1Reloaded = new ConfigProperty('shared', 'initial', undefined, undefined, mockStorageInterface);
        ...
        it('should not emit changes when update is rejected due to older timestamp', () => {
            const config = new ConfigProperty('test', 'initial', undefined, undefined, mockStorageInterface);
            ...
            const subscription = config.changes$.subscribe(value => {
            ...
            // This should be accepted (newer timestamp)
            config.setValue('value3', 1500);

            subscription.unsubscribe();

            // Should have emissions for value1 (initial) and value3, but not value2
            expect(emissions).toContain('value1');
            expect(emissions).toContain('value3');
            expect(emissions).not.toContain('value2');
        });

        it('should handle array values with timestamps correctly', () => {
            const config = new ConfigProperty<number[]>('test', [], undefined, undefined, mockStorageInterface);
    ...
    describe('Error handling', () => {
        it('should handle corrupted stored values gracefully', () => {
            ...
            const consoleSpy = vi.spyOn(console, 'error');
            const config = new ConfigProperty('test', { default: true }, undefined, undefined, mockStorageInterface);
        ...
        it('should handle missing storage gracefully', () => {
            ...
            const consoleSpy = vi.spyOn(console, 'error');
            const config = new ConfigProperty('test', 'default', undefined, undefined, brokenStorage);
