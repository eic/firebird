// /artifacts/sim_dis_10x100_minQ2=1000_epic_craterlake.edm4hep.root/sim_dis_10x100_minQ2=1000_epic_craterlake.edm4hep.root
...
class TExampleSelector extends TSelector {
  ...
  constructor() {
    super();
    this['addBranch']('B0ECalHits');
  }

  /** function called before reading of TTree starts */
  Begin() {
    console.log('Start reading TTree');
  }

  /** function called for every entry */
  Process() {
    console.log(this['tgtobj'].B0ECalHits);
  }
  ...
  Terminate(res:any) {
    if (!res ) {
      ...
      res = `Procesed the tree`;
    }

    console.log(`Start reading Terminate: ${res}`);
  }
}
...
/**
 * Edm4hepRootEventLoader for loading EDM4hep CERN ROOT format
 */
export class Edm4hepRootEventLoader extends PhoenixLoader {
  /**  Event data loaded from EDM4hep JSON file */
  private rawEventData: any;
  private rootFile: any;
  ...
  async openFile(url: any) {
    console.log(`Edm4hep: Loading file ${url}`)
    console.time('Edm4hep: Open root file');
    const file = await openFile(url);
    console.log(file);
    console.timeEnd('Edm4hep: Open root file');
    this.rootFile = file;

    console.time('Edm4hep: Getting EDM4HEP tree');
    const obj = await file.readObject("events");
    console.timeEnd('Edm4hep: Getting EDM4HEP tree');
    console.log(obj);
    console.dir(obj);
    this.rootTree = obj;

    treeProcess(obj, new TExampleSelector(), 1, 1);
    //"B0ECalHits"
  }
