...
export class ThreeService implements OnDestroy {
  ...
  /** A ResizeObserver to detect size changes of the container */
  ...
  init(containerId: string): void {
    ...
    // 5) Use ResizeObserver instead of window resize
   ...
   * This ensures both cameras and the renderer are updated.
   */
  onResize(): void {
    if (!this.initialized) {
      return; // or throw an error if you'd rather
    }

    const width = this.renderer.domElement.parentElement?.clientWidth ?? 800;
    const height =
      this.renderer.domElement.parentElement?.clientHeight ?? 600;
  ...
  /**
   * On service destroy, stop the render loop and disconnect ResizeObserver
   */
  ngOnDestroy(): void {
    this.stopRendering();
